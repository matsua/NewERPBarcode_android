...
public class InMemoryTreeStateManager<T> implements TreeStateManager<T> {
    ...
    private transient List<T> allNodeListCache = null;
    ...
    public synchronized void addAfterChild(final T parent, final T newChild, final T afterChild) {
        ...
        if (afterChild == null) {
            ...
            final InMemoryTreeNode<T> added = node.add(
                    index == -1 ? node.getChildrenListSize() : index + 1, newChild, visibility);
            allNodes.put(newChild, added);
        }
        if (visibility) {
            internalDataSetChanged();
    ...
    public synchronized List<T> getAllNextNodeList() {
    	allNodeListCache = new ArrayList<T>(allNodes.size());
        T currentId = null;
        do {
            ...
            if (currentId == null) {
            	...
            	allNodeListCache.add(currentId);
            }
        } while (true);
        return allNodeListCache;
    }

    public synchronized T getNext(final T id) {
        final InMemoryTreeNode<T> node = getNodeFromTreeOrThrowAllowRoot(id);
        //if (!node.isVisible()) {
        //    return null;
        //}
