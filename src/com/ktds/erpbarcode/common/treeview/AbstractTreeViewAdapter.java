package com.ktds.erpbarcode.common.treeview;
...
public abstract class AbstractTreeViewAdapter<T> extends BaseAdapter implements ListAdapter {
    ...
    public AbstractTreeViewAdapter(final Context context, final TreeStateManager<T> treeStateManager, final int numberOfLevels) {
    ...
    protected int getTreeListItemWrapperId() {
        return R.layout.barcode_tree_wrapper;
    }
    ...
    private Drawable getDrawableOrDefaultBackground(final Drawable r) {
        if (r == null) {
            return mContext.getResources().getDrawable(R.drawable.treeview_sb).mutate();
        } else {
            return r;
        }
    }
	...
	public final LinearLayout populateTreeItem(final LinearLayout layout, final View childView, 
        ...
        //layout.setBackground(individualRowDrawable == null ? getDrawableOrDefaultBackground(rowBackgroundDrawable) : individualRowDrawable);
        ...
        final FrameLayout.LayoutParams childParams = new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
        if (newChildView) {
